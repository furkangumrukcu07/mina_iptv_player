...
class BetterPlayerPlugin : FlutterPlugin, ActivityAware, MethodCallHandler {
    ...
    private fun disposeAllPlayers() {
        val n = videoPlayers.size
        Log.d(SURFACE_LOG_TAG, "disposeAllPlayers count=$n (engine init / detach)")
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            ...
            CREATE_METHOD -> {
                ...
                Log.d(
                    SURFACE_LOG_TAG,
                    "CREATE new player textureId=$tid mapSize=${videoPlayers.size} (SurfaceProducer allocated)",
                )
            }
...
//                    )
    ...
    private fun dispose(player: BetterPlayer, textureId: Long) {
        ...
        Log.d(
            SURFACE_LOG_TAG,
            "DISPOSE method channel textureId=$textureId mapSizeBefore=$before",
        )
        player.dispose()
        ...
        Log.d(
            SURFACE_LOG_TAG,
            "DISPOSE done textureId=$textureId mapSizeAfter=${videoPlayers.size}",
        )
        stopPipHandler()
    }
