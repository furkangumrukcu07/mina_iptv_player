...
class BetterPlayerController {
    ...
    if (data != null) {
      ...
      if ((betterPlayerDataSource?.useAsmsAudioTracks ?? false) && _isDataSourceAsms(betterPlayerDataSource!)) {
        ...
        if (_betterPlayerAsmsAudioTracks?.isNotEmpty ?? false) {
          final list = _betterPlayerAsmsAudioTracks!;
          var chosen = list.first;
          // Telefonlarda AC3/EAC3/DTS çoğu zaman çalınamaz; AAC/Opus tercih et.
          if (Platform.isAndroid) {
            int score(BetterPlayerAsmsAudioTrack t) {
              final s =
                  '${t.label ?? ''} ${t.language ?? ''} ${t.mimeType ?? ''}'
                      .toLowerCase();
              var n = 0;
              if (s.contains('aac') ||
                  s.contains('mp4a') ||
              ...
              if (s.contains('mp3') || s.contains('mpeg')) {
                n += 6;
              }
              if (s.contains('ac3') ||
                  s.contains('ac-3') ||
            ...
            for (final t in list) {
              if (score(t) > score(chosen)) {
                chosen = t;
              }
            }
          }
          try {
            setAudioTrack(chosen);
          } catch (_) {
            // VideoPlayer henüz hazır değilse veya parça uyuşmazlığı — sessiz geç.
          }
        }
      }
    }
  ...
  /// Stops platform playback (buffer flush); see [VideoPlayerController.stop].
  ...
  void setAudioTrack(BetterPlayerAsmsAudioTrack audioTrack) {
    ...
    // language null olsa bile id varsa native seçim yapılmalı (çoğu yayında dil etiketi boş).
    videoPlayerController!.setAudioTrack(
      audioTrack.label ?? audioTrack.language ?? 'audio',
      audioTrack.id,
    );
  }
