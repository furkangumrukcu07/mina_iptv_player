...
import 'package:better_player_plus/src/core/better_player_surface_log.dart';
...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  Future<void> _setDataSource(DataSource dataSourceDescription) async {
    ...
    BetterPlayerSurfaceLog.setDataSourceSameTexture(_textureId);
  ...
  /// Stops playback on the platform (e.g. ExoPlayer [stop]) and cancels position updates.
  ...
  void _updatePosition(Duration? position, {DateTime? absolutePosition}) {
    final newPosition = _seekPosition ?? position;
    final newAbsolute = _seekPosition == null
        ? absolutePosition
        : value.absolutePosition;
    if (newPosition == value.position && newAbsolute == value.absolutePosition) {
      return;
    }
