...
class PlayerController extends GetxController with WidgetsBindingObserver {
  ...
  String _normalizePlaybackStreamUrl(String raw) => IptvPlaybackDefaults.normalizeStreamUrl(
        raw,
  ...
  void requestOpenLiveChannelStripFromTvOsd() {
    // Kanal degiiminde channel strip otomatik açilmasin
  ...
  void shiftLiveStripCategory(int delta) {
    ...
    final n =
        (liveStripCategoryIndex.value + delta).clamp(0, tapes.length - 1);
  ...
  Future<void> applyMediaKitLibmpvPlaybackOptions(Player player) async {
    ...
    try {
      ...
      await plat.setProperty('video-output-levels', 'full');
      await plat.setProperty('colormatrix', 'auto');

      final cores = Platform.numberOfProcessors;
      final lavcThreadsGeneric = cores > 0
          ? math.min(4, math.max(1, cores))
          : 4;
      ...
      if (Platform.isAndroid) {
        ...
        if (swPurpleFix) {
          ...
          await plat.setProperty('hwdec', hwdec);
          hwdecLog = hwdec;
        }
        await plat.setProperty('framedrop', weak ? 'yes' : 'vo');

        final lavcThreads =
            weak ? math.min(2, lavcThreadsGeneric) : lavcThreadsGeneric;
        await plat.setProperty('vd-lavc-threads', '$lavcThreads');
        ...
        if (aml) {
          ...
          await plat.setProperty('demuxer-max-bytes', '16M');
          await plat.setProperty('demuxer-max-back-bytes', '8M');
          if (_currentStreamIsLive) {
            await plat.setProperty('video-latency-hacks', 'yes');
          }
        } else {
        ...
        await plat.setProperty('hwdec', 'auto-safe');
      ...
      await plat.setProperty('scale', 'bilinear');
      await plat.setProperty('cscale', 'bilinear');
      await plat.setProperty('dscale', 'bilinear');
  ...
  Future<void> ensureOrphanBetterBootRetry() async {
    ...
    try {
      ...
      final delayMs = _settings.layoutMode.value == AppLayoutMode.tv ? 900 : 1000;
  ...
  void zapRelativeDebounced(int delta) {
    ...
    _zapRelativeDebounceTimer?.cancel();
    final delay =
        _currentStreamIsLive ? _zapDebounceLive : _zapDebounceDefault;
  ...
  void scheduleTvOsdAutoHide() {
    ...
    final customDuration = Duration(seconds: _settings.tvOsdAutoHideDuration.value);
  ...
  void _syncLiveAutoNextWatchdog() {
    ...
    _liveAutoNextPollTimer ??=
        Timer.periodic(_liveAutoNextPollInterval, (_) {
      if (isClosed) {
        _cancelLiveAutoNextWatchdog();
        return;
      }
  ...
  bool _scheduleXtreamGetPhpToVodPathRetry() {
    ...
    _playUrlOverride = converted;
    error.value = null;
    debugPrint('mina_iptv: get.php başarısız → segment yolu deneniyor: $converted');
    unawaited(_boot(preferredMaxHeight: null, disableAsms: false));
    return true;
  }
  ...
  Future<void> _boot({
    ...
    final int? effectivePreferredMaxHeight = disableAsms
        ? null
        : (preferredMaxHeight ??
            _preferredMaxVideoHeightForAdaptivePlayback());
    try {
      ...
      final cfg = IptvBetterPlayerConfig.playerConfiguration(
        ...
        deviceOrientationsAfterFullScreen:
            IptvBetterPlayerConfig.mobileBetterPlayerAfterFullscreenOrientations(
          layoutMode,
          _settings,
        ),
  ...
  void _startWatchProgressSaver() {
    ...
    if (_currentStreamIsLive) return;
    _watchProgressTimer =
        Timer.periodic(const Duration(seconds: 12), (_) => _persistWatchProgressTick());
  }

  void _persistWatchProgressTick() {
    if (isClosed) return;
    if (_currentStreamIsLive) return;
    ...
    if (posMs >= dur.inMilliseconds * 0.95) {
      unawaited(wp.clear(id));
      // Also remove from ContinueWatchingService
      if (Get.isRegistered<ContinueWatchingService>()) {
        Get.find<ContinueWatchingService>().removeItem(id);
      }
      return;
    }
    unawaited(wp.saveProgress(id, posMs, dur.inMilliseconds));
    // Also save to ContinueWatchingService for VOD
    if (Get.isRegistered<ContinueWatchingService>()) {
      final cw = Get.find<ContinueWatchingService>();
      cw.saveProgress(
        vodId: id,
  ...
  Future<void> _maybeOfferVodResume() async {
    ...
    bool sameChannel() =>
        !isClosed && channel.value.id == anchorChannelId;
  ...
  void _scheduleAndroidVodAudioFix(BetterPlayerController c, int bootGen) {
    ...
    void trySingleRiskyAsmsAudioFallbackToMediaKit() {
      ...
      unawaited(_vodSilentAudioFallbackToMediaKit('vod_single_risky_asms_audio'));
    }
  ...
  bool _maybeRetryBetterWithSoftwareDecoderBeforeMediaKit(String msg) {
    ...
    if (!_isPlaybackDecoderFailure(msg)) return false;
    if (!_currentStreamIsLive && !_shouldAutoFallbackToMediaKit(msg)) return false;
...
class InAppBrightnessBetterOverlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      final dim =
          (1.0 - pc.inAppPlaybackBrightness.value).clamp(0.0, 1.0);
