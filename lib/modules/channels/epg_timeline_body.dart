...
class _ChannelsEpgTimelineBodyState extends State<ChannelsEpgTimelineBody> {
  ...
  String _programmeStartLabelForRow(
    ...
    final inW = epg.programmesInWindowForLiveChannel(ch, w0, w1);
    if (inW.isNotEmpty) return _fmtHm(inW.first.start);
    return '--:--';
  }
...
class _EpgProgrammeRowGlass extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final list = epg.programmesInWindowForLiveChannel(channel, windowStart, windowEnd);
    final nowX = xAt(now);
    final timeFmt =
