...
abstract final class PlaybackOrientationManager {
  ...
  static AppSettingsService? get _app => Get.isRegistered<AppSettingsService>()
  ...
  static Future<void> forcePortraitMobilePlayback() async {
    ...
    app.mobilePlaybackLandscapeUserLocked.value = false;
  ...
  static Future<void> forceLandscapeMobilePlayback() async {
    ...
    app.mobilePlaybackLandscapeUserLocked.value = true;
    await SystemChrome.setPreferredOrientations(const <DeviceOrientation>[
  ...
  /// Mobil modda zorunlu yatay/dikey geçiş toggle
  static Future<void> toggleMobileForcedOrientation() async {
    ...
    if (app.mobilePlaybackPortraitUserLocked.value) {
      ...
      await forceLandscapeMobilePlayback();
    } else if (app.mobilePlaybackLandscapeUserLocked.value) {
      // Yatay kilitli -> portreye zorla
      await forcePortraitMobilePlayback();
    } else {
      // Kilit yok -> mevcut duruma göre tersine zorla
      final views = WidgetsBinding.instance.platformDispatcher.views;
      if (views.isNotEmpty) {
        final ps = views.first.physicalSize;
        if (ps.width > ps.height) {
          // Şu an yatay -> portreye zorla
          await forcePortraitMobilePlayback();
        } else {
          // Şu an dikey -> yataya zorla
          await forceLandscapeMobilePlayback();
        }
      } else {
        // Belirsiz -> yataya zorla
        await forceLandscapeMobilePlayback();
      }
    }
  }
